...
class DetailsFragment : BottomSheetDialogFragment() {
    ...
    private val datePicker by lazy {
        val picker = TimePickerBuilder(context) { date, v ->
            ...
            binding.date.setKV("开始日期", taskModel.beginTime.yyyyMd)
        }
    ...
    private val endDatePicker by lazy {
        val picker = TimePickerBuilder(context) { date, v ->
            Log.i("pvTime", "onTimeSelect")
            val dEndTime = taskModel.endTime - beginOfDay(taskModel.endTime).timeInMillis
            (taskModel as? DailyTaskModel)?.changeEndTime(
            ...
            binding.endDate.setKV("结束日期", taskModel.endTime.yyyyMd)
        }
            .setType(booleanArrayOf(true, true, true, false, false, false))
            .isDialog(true) //默认设置false ，内部实现将DecorView 作为它的父控件。
            .addOnCancelClickListener { Log.i("pvTime", "onCancelClickListener") }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.endDate.setOnClickListener {
            endDatePicker.setDate(taskModel.endTime.calendar)
            endDatePicker.show(it)
        }
        ...
        binding.save.setOnClickListener {
            lifecycleScope.launch {
                if (taskModel.duration <= 0) {
                    Toast.makeText(requireContext(), "结束时间必须大于开始时间", Toast.LENGTH_SHORT)
                        .show()
                    return@launch
                }
