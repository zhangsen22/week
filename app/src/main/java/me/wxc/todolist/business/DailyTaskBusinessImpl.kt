...
import me.wxc.widget.schedule.isFullDay
import java.util.*
...
class DailyTaskBusinessImpl : DailyTaskBusiness {
    ...
    override suspend fun getDailyTasks(
        ...
        val fullDays = repo.getAllDailyTasks().map { it.mapModel }.filter { it.isFullDay }.apply {
            Log.i(TAG, "getDailyTasks: ${joinToString { it.title }}")
        }
        repo.getRangeDailyTasks(beginTime, endTime).map {
            it.mapModel
        } - fullDays.toSet() + fullDays
    }
            ...
            when (option) {
                UpdateOptionFragment.UpdateOption.ONE -> {
                    ...
                    refreshCache(
                        model.beginTime,
                        model.endTime,
                        listOf(model.copy(calendarEventId = newEventId))
                    )
